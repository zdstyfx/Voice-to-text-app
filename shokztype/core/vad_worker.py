...
class VadTranscriptionWorker:
    ...
    def __init__(
        ...
        self._cloud_streaming = self._asr_backend in ("cloud", "volcengine") and on_partial is not None
    ...
    def _kws_to_active(self) -> None:
        ...
        将 _recent_frames 中积攒的帧重新喂给 VAD，避免紧跟唤醒词
        的语音因 VAD 需要积累帧数而被吞掉开头。
        """
        ...
        # 把缓冲区内的帧预喂给 VAD，让它提前积累语音检测状态
        for frame in self._recent_frames:
            results = self._vad.process_frame(frame)
            for r in results:
                if r.is_speech_start and not self._in_speech:
                    self._on_speech_start()
                if r.is_speech_end and self._in_speech:
                    self._on_speech_end()
            if self._in_speech:
                self._speech_buffer.append(frame)

        logger.info("KWS: entering ACTIVE mode (pre-fed %d frames to VAD)", len(self._recent_frames))
        try:
            import winsound
