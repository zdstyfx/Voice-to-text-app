...
# keywords.txt 路径
_KEYWORDS_PATH = os.path.join(_PROJECT_ROOT, "keywords.txt")
...
async def add_start_keyword(request: Request) -> dict:
    """添加一个开始关键词。复用 core.kws_add_keyword 的 text2token 转换。"""
    ...
    if not os.path.isabs(model_dir):
        model_dir = os.path.join(_PROJECT_ROOT, model_dir)
    tokens_path = os.path.join(model_dir, "tokens.txt")

    try:
        line = text_to_kws_line(keyword, tokens_path)
    except (ValueError, FileNotFoundError) as e:
        return {"success": False, "error": str(e)}

    with open(_KEYWORDS_PATH, "a", encoding="utf-8") as f:
        f.write(line + "\n")
