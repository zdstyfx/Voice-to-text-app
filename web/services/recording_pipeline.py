...
def _on_result(result: TranscriptionResult) -> None:
    ...
    # 声纹过滤：检查说话人是否在当前激活列表中
    config = get_config()
    vp_cfg = config.get("voiceprint", {})
    if vp_cfg.get("enabled") and result.speaker:
        active = vp_cfg.get("activeProfiles", [])
        if active and result.speaker not in active:
            logger.info("声纹过滤: '%s' 不在激活列表 %s，跳过", result.speaker, active)
            return
...
def restart_pipeline() -> dict:
    ...
    global _worker, _ready, _init_error, _recording, _wakeup_method, _first_active_result
